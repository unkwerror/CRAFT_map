...
Статистика (проценты по округам и категориям, как на слайдах) — SQL view поверх objects, не хранить руками:

```sql
create view district_stats as
select d.name, count(o.id) as cnt,
  round(100.0 * count(o.id) / sum(count(o.id)) over (), 2) as pct
from districts d left join objects o
  on o.district_id = d.id and o.published
group by d.name;
```
...
API: `GET /api/objects` → GeoJSON FeatureCollection (только published, поля: id, title, category, thumb). `GET /api/objects/[id]` → полная карточка. `GET /api/districts` → полигоны. `GET /api/stats` → district_stats.
...
5. Панель фильтров: 4 чекбокса-категории с цветами + выпадающий список округов. Фильтрация на клиенте через setFilter, без перезапросов
6. Блок статистики: проценты по округам (горизонтальные плашки, как на референсе), обновляется при фильтрации
7. Клик по названию округа → fitBounds на его полигон
8. Mobile-first: карта на весь экран, фильтры в шторке снизу
...
4. **Интерактив:** фильтры, кластеры, карточки объектов, статистика, mobile
...
- Не считать проценты статистики на клиенте руками — только из view
