...
export async function POST(req:NextRequest){
 ...
 try{const result=await pg.begin(async sql=>{const [route]=await sql<{id:string}[]>`insert into routes(slug,status,title,summary,description,cover_url,theme,mode,estimated_duration_minutes,distance_meters,difficulty,age_group,accessibility_profile,published_at,created_by) values(${d.slug},'draft',${d.title},${d.summary??null},${d.description??null},${d.coverUrl??null},${d.theme??null},${d.mode},${d.estimatedDurationMinutes??null},${d.distanceMeters??null},${d.difficulty??null},${d.ageGroup??null},${JSON.stringify(d.accessibilityProfile)}::jsonb,${null},${guard.session.user.id}) returning id`;if(!route)throw new Error('no route');for(const [i,s] of d.stops.entries())await sql`insert into route_stops(route_id,object_id,position,arrival_radius_meters,recommended_duration_minutes,intro_text,directions_text,gps_autoplay) values(${route.id},${s.objectId},${i+1},${s.arrivalRadiusMeters},${s.recommendedDurationMinutes??null},${s.introText??null},${s.directionsText??null},${s.gpsAutoplay})`;return route});return NextResponse.json(result,{status:201})}catch(error){console.error(error);return NextResponse.json({error:'Не удалось создать маршрут. Проверьте уникальность slug.'},{status:409})}
}
